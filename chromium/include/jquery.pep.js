...
;(function ( $, window, undefined ) {
  ...
  Pep.prototype.xToBorder= function() {
		if($(window).width() <= this.$el.width())return;		
		var xDifferent = $(window).width() - this.options.startPos.left;
		var newX = xDifferent - this.$el.width();	
    this.cssX = newX;
		var matrixArray  = this.matrixToArray( this.matrixString());
    matrixArray[4]    = newX;
    this.translation  = this.arrayToMatrix( matrixArray );
  ...
  Pep.prototype.yToBorder= function() {
		if($(window).height() <= this.$el.height())return;
		var yDifferent = $(window).height() - this.options.startPos.top;
		var newY = yDifferent - this.$el.height();
    this.cssY = newY;
		var matrixArray  = this.matrixToArray( this.matrixString());
    matrixArray[5]    = newY;
    this.translation  = this.arrayToMatrix( matrixArray );
  ...
  Pep.prototype.handleConstraint = function(dx, dy, accountForTranslation) {
    ...
    if ( $.isArray( this.options.constrainTo ) ) {
      ...
      upperXLimit       = this.$container.width()  - this.$el.outerWidth();
			if(this.options.constrainTo === 'window'){
				upperYLimit       = $(window).height() - this.$el.outerHeight();
			}else{
