...
KeyboardVisual.prototype.keyFunctDownByCode = function(code){
  if(this.toShowOption != 'newer')
		for(var i = this.kb.keyFunctionals.length-1; i > -1 ; i--){
...
KeyboardVisual.prototype.keyFunctUpByCode = function(code){
  if(this.toShowOption != 'newer')
		for(var i = this.kb.keyFunctionals.length-1; i > -1 ; i--){
			if( this.kb.keyFunctionals[i].code == code ){
...
KeyboardVisual.prototype.keyLetterUpByCode = function(code){
  if(this.toShowOption != 'newer')
    for(var i = this.kb.keyLetters.length-1; i > -1 ; i--)
      if( this.kb.keyLetters[i].code == code )
  			this.kb.keyLetters[i].visual.up();
}


KeyboardVisual.prototype.keyLangaugeNextPressing = function(){
	if(this.toShowOption == 'newer')
		return;

  this.keyNextLanguage.down();
  var that = this;
  setTimeout(function(){
    that.keyNextLanguage.up();
  }, 250);
}
